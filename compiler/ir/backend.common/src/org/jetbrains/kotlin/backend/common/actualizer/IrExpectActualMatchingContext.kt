...
abstract class IrExpectActualMatchingContext(
    ...
    override val ClassLikeSymbolMarker.typeParameters: List<TypeParameterSymbolMarker>
        get() = safeAsIr<IrTypeParametersContainer>()?.typeParameters?.map { it.symbol }.orEmpty()
    ...
    private inner class ActualizingSubstitutor : AbstractIrTypeSubstitutor() {
        ...
        private fun substituteOrNull(type: IrType): IrType? {
            ...
            val newClassifier = (type.classifier.owner as? IrClassLikeDeclaration)
                ?.let { expectToActualClassMap[it.classIdOrFail] }
